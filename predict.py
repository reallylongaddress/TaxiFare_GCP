...
def get_test_data(data="gcp", nrows=1000):
    ...
    if data == "local":
        ...
        print(f'get_test_data GCP: gs://{gcm_params.BUCKET_NAME}/{gcm_params.BUCKET_TEST_DATA_PATH}')
        df = pd.read_csv(f"gs://{gcm_params.BUCKET_NAME}/{gcm_params.BUCKET_TEST_DATA_PATH}")
    else:
